...
static void InitTimer(void)
{
    ...
    #if defined(__linux__) && !defined(__VOS__)
        struct timespec now;
    ...
    #if defined(__VOS__)
        frequency = 1000;  // VOS sys_uptime() returns milliseconds
    #endif
...
static uint64_t GetTimeCount(void)
{
    ...
    #if defined(__VOS__)
        // VOS: use sys_uptime() which returns milliseconds
        extern unsigned int sys_uptime(void);
        value = (uint64_t)sys_uptime();
    #endif
