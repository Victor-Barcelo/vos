...
static bool syscmd_append(char **dst, size_t *len, size_t *cap, const char *src, size_t src_len) {
	if (!dst || !len || !cap || !src || src_len == 0) return true;
	size_t need = *len + src_len + 1;
	if (need > *cap) {
		size_t new_cap = *cap ? *cap : 64;
		while (new_cap < need) new_cap *= 2;
		char *p = realloc(*dst, new_cap);
		if (!p) {
			free(*dst);
			*dst = NULL;
			*len = 0;
...
static bool syscmd_append_cstr(char **dst, size_t *len, size_t *cap, const char *src) {
	if (!src) return true;
	return syscmd_append(dst, len, cap, src, strlen(src));
}

static bool syscmd_append_char(char **dst, size_t *len, size_t *cap, char c) {
	return syscmd_append(dst, len, cap, &c, 1);
}

static char *syscmd_expand_placeholders(const buffer *b, const char *cmd) {
	if (!cmd) return NULL;

	const char *filename = b ? b->filename : NULL;
	const char *base = filename ? file_part(filename) : NULL;
	const size_t base_len = base ? strlen(base) : 0;

	size_t dir_len = 0;
	if (filename && base && base > filename) dir_len = (size_t)(base - filename);

	const char *dot = NULL;
	if (base) {
		for (const char *p = base + base_len; p > base; p--) {
			if (p[-1] == '.') {
				dot = p - 1;
				break;
			}
	...
	for (const char *p = cmd; *p; p++) {
		if (*p != '%') {
			if (!syscmd_append_char(&out, &out_len, &out_cap, *p)) goto oom;
			continue;
		}

		const char next = p[1];
		if (next == 0) {
			if (!syscmd_append_char(&out, &out_len, &out_cap, '%')) goto oom;
			break;
		}
		p++;
		...
		switch (next) {
			case '%':
				...
				break;
			case 'f':
				if (filename && !syscmd_append_cstr(&out, &out_len, &out_cap, filename)) goto oom;
				break;
			case 'd':
				if (filename && dir_len && !syscmd_append(&out, &out_len, &out_cap, filename, dir_len)) goto oom;
				break;
			case 'n':
				if (base && !syscmd_append(&out, &out_len, &out_cap, base, base_len)) goto oom;
				break;
			case 'b':
				if (base && !syscmd_append(&out, &out_len, &out_cap, base, stem_len)) goto oom;
				break;
			case 'e':
				if (ext && ext_len && !syscmd_append(&out, &out_len, &out_cap, ext, ext_len)) goto oom;
				break;
			default:
				if (!syscmd_append_char(&out, &out_len, &out_cap, '%')) goto oom;
				if (!syscmd_append_char(&out, &out_len, &out_cap, next)) goto oom;
				break;
		}
	}
	...
	case GOTOBOOKMARK_A:
		{
			...
			case GOTOBOOKMARK_A:
				if (! (b->bookmark_mask & (1 << c))) return BOOKMARK_NOT_SET;
					...
					if (relative) {
						snprintf(msg, MAX_MESSAGE_SIZE, "At Bookmark %c", "0123456789-<>w"[c]);
						print_message(msg);
					}
				}
	...
	case NAMECONVERT_A:
		...
		case SYSTEM_A:
			if (p || (p = request_string(b, "Shell command", NULL, false, COMPLETE_FILE, b->encoding == ENC_UTF8 || b->encoding == ENC_ASCII && b->opt.utf8auto))) {
				...
				char *expanded = syscmd_expand_placeholders(b, p);
				if (expanded) {
					free(p);
					p = expanded;
				}
