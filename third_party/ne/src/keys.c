 ...
 * Handle SGR mouse (CSI <b;x;yM/m) and legacy X10 mouse (CSI M b x y). */

static int vos_handle_mouse_event(int button_code, int x1, int y1, bool press) {
	(void)x1;
	(void)y1;
	...
	/* Wheel buttons map to PageUp/PageDown by default. */
	if ((button_code & 0xC0) == 0x40) {
	...
	if (line < 0) line = 0;
	if (line >= b->num_lines) line = b->num_lines - 1;

	line_desc * const ld = nth_line_desc(b, line);
	if (!ld) {
		return INVALID_CHAR;
	}

	int64_t virt_x = b->win_x + col;
	if (virt_x < 0) virt_x = 0;
	const int64_t pos = calc_virt_pos(ld, virt_x, b->opt.tab_size, b->encoding);
	goto_line_pos(b, line, pos);
	return INVALID_CHAR;
}

// Returns: 0 not a mouse sequence, 1 handled (out_code set), 2 needs more data.
static int vos_try_handle_mouse(char *kbd_buffer, int *cur_len, int *out_code) {
	if (!kbd_buffer || !cur_len || !out_code) {
		return 0;
	}
	if (*cur_len < 3) {
		return 0;
	}
	if ((unsigned char)kbd_buffer[0] != 0x1B || kbd_buffer[1] != '[') {
		return 0;
	}
	...
	if (kbd_buffer[2] == 'M') {
		...
		int b = (int)(unsigned char)kbd_buffer[3] - 32;
		int x = (int)(unsigned char)kbd_buffer[4] - 32;
		int y = (int)(unsigned char)kbd_buffer[5] - 32;
		bool press = b != 3;
		int rc = vos_handle_mouse_event(b, x, y, press);
		memmove(kbd_buffer, kbd_buffer + 6, (size_t)(*cur_len - 6));
		*cur_len -= 6;
		*out_code = rc;
		return 1;
	}
	...
	/* SGR: ESC [ < b ; x ; y M/m */
	int end = -1;
	for (int i = 3; i < *cur_len; i++) {
		if (kbd_buffer[i] == 'M' || kbd_buffer[i] == 'm') {
			end = i;
			break;
		}
	}
	...
	bool have = false;
	int b = 0, x = 0, y = 0;
	bool press = kbd_buffer[end] == 'M';
	bool ok = true;

	for (int i = 3; i < end; i++) {
		char ch = kbd_buffer[i];
		if (ch >= '0' && ch <= '9') {
			val = val * 10 + (ch - '0');
			have = true;
			continue;
	...
	int rc = INVALID_CHAR;
	if (ok) {
		rc = vos_handle_mouse_event(b, x, y, press);
	}

	int consumed = end + 1;
	memmove(kbd_buffer, kbd_buffer + consumed, (size_t)(*cur_len - consumed));
	*cur_len -= consumed;
	*out_code = rc;
	return 1;
}
...
int get_key_code(void) {
	...
	while(true) {
		...
		if (cur_len) {
			/* First, try to consume and handle xterm mouse reporting. */
			int mouse_code = 0;
			int mouse_res = vos_try_handle_mouse(kbd_buffer, &cur_len, &mouse_code);
			if (mouse_res == 1) {
				return mouse_code;
			}
			if (mouse_res == 2) {
