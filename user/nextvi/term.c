...
void term_pos(int r, int c)
{
	char buf[64] = "\r\33[", *s;
	if (r < 0) {
		memcpy(nvi_itoa(MAX(0, c), buf+3), c > 0 ? "C" : "D", 2);
		term_out(buf);
	} else {
		s = nvi_itoa(r + 1, buf+3);
		if (c > 0) {
			*s++ = ';';
			s = nvi_itoa(c + 1, s);
		}
	...
	if (SYN_FGSET(att)) {
		*s++ = ';';
		if (fg < 8)
			...
			s = nvi_itoa(fg, (char*)memcpy(s, "38;5;", 5)+5);
	}
	if (SYN_BGSET(att)) {
		*s++ = ';';
		if (bg < 8)
			...
			s = nvi_itoa(bg, (char*)memcpy(s, "48;5;", 5)+5);
	}
	s[0] = 'm';
	s[1] = '\0';
	return buf;
