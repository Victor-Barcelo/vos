		...
		if (mv > 0) {
			...
			case 'v':
				...
				case 'i':;
					...
					if (k == 'I') {
						...
						strcpy(restr, "%s/^ {");
						strcpy(nvi_itoa(vi_arg, restr+6), "}/\t/g");
					}
				...
				case 't': {
					...
					strcpy(buf, ".,.+");
					char *buf1 = nvi_itoa(vi_arg, buf+4);
					strcat(buf1, "s/");
					if (cs) {
						strcat(buf1, cs);
						strcat(buf1, "/");
			...
			case 'g':
				...
				if (k == 'g')
					...
					preserve(int, xled, xled = 0;)
					char cmd[64] = "g/./& ";
					strcpy(nvi_itoa(vi_arg, cmd+5), "gw");
					ex_command(cmd)
					restore(xled)
					vi_mod |= 1;
				} else if (k == '~' || k == 'u' || k == 'U') {
